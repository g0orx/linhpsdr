...
GtkWidget *create_radio_dialog(RADIO *radio) {
  ...
  GtkWidget *smeter_frame=gtk_frame_new("S-Meter Calibration");
  GtkWidget *smeter_grid=gtk_grid_new();
  gtk_grid_set_row_homogeneous(GTK_GRID(smeter_grid),TRUE);
  gtk_grid_set_column_homogeneous(GTK_GRID(smeter_grid),FALSE);
  gtk_container_add(GTK_CONTAINER(smeter_frame),smeter_grid);
  gtk_grid_attach(GTK_GRID(grid),smeter_frame,col,row++,1,1);

  GtkWidget *smeter_label=gtk_label_new(" S-Meter:");
  gtk_grid_attach(GTK_GRID(smeter_grid),smeter_label,0,1,1,1);
  ...
  gtk_grid_attach(GTK_GRID(smeter_grid),smeter_scale,1,1,1,1);
